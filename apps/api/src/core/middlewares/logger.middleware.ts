...
export class LoggerMiddleware implements NestMiddleware {
    ...
    use(req: Request, res: Response, next: () => void): void {
        ...
        res.on("close", () => {
            ...
            this.logger.debug(`[${hostname}] "${method} ${greenPath}" ${String(statusCode)} ${statusMessage} ${String(contentLength)} "${referer}" "${userAgent}" "${String(ip)}" Headers: ${JSON.stringify(responseHeaders)}`);
