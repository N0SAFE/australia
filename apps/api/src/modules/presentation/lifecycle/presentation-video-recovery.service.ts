...
/**
 * Presentation Video Recovery Service
 *
 * Handles crash recovery for video processing in the Presentation module.
 ...
 */
@Injectable()
export class PresentationVideoRecoveryService implements OnModuleInit {
  private readonly logger = new Logger(PresentationVideoRecoveryService.name);
  ...
  private async recoverDanglingFiles(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Found ${String(danglingFiles.length)} dangling file(s) in presentation/video namespace`,
      );
  ...
  private async processDanglingFile(file: {
    ...
    try {
      ...
      if (!dbFile) {
        ...
        await this.ffmpegService.cleanup(fileId, [...PRESENTATION_VIDEO_NAMESPACE]);
        return;
      }
      ...
      // Cleanup on error
      await this.ffmpegService.cleanup(fileId, [...PRESENTATION_VIDEO_NAMESPACE]).catch(() => {
  ...
  private async recoverCompletedFile(
    ...
    try {
      ...
      // Cleanup temp files
      await this.ffmpegService.cleanup(danglingFile.fileId, [...PRESENTATION_VIDEO_NAMESPACE]);
