...
/**
 * Storage Video Recovery Service
 *
 * Handles crash recovery for video processing in the Storage module.
 ...
 */
@Injectable()
export class StorageVideoRecoveryService implements OnModuleInit {
  private readonly logger = new Logger(StorageVideoRecoveryService.name);
  ...
  private async recoverDanglingFiles(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Found ${String(danglingFiles.length)} dangling file(s) in storage namespace`,
      );
  ...
  private async processDanglingFile(file: {
    ...
    try {
      ...
      if (!dbFile) {
        ...
        await this.ffmpegService.cleanup(fileId, [...STORAGE_VIDEO_NAMESPACE]);
        return;
      }
      ...
      // Cleanup on error
      await this.ffmpegService.cleanup(fileId, [...STORAGE_VIDEO_NAMESPACE]).catch(() => {
  ...
  private async recoverCompletedFile(
    ...
    try {
      ...
      // Cleanup temp files
      await this.ffmpegService.cleanup(danglingFile.fileId, [...STORAGE_VIDEO_NAMESPACE]);
