...
class ProgressSubscription {
  ...
  subscribe(callback: ProgressSubscriber): () => void {
    ...
    console.log(`[ProgressPlugin] Total general subscribers: ${this.subscribers.size}`);
  ...
  subscribeUpload(callback: ProgressSubscriber): () => void {
    ...
    console.log(`[ProgressPlugin] Total upload subscribers: ${this.uploadSubscribers.size}`);
    return () => {
  ...
  subscribeDownload(callback: ProgressSubscriber): () => void {
    ...
    console.log(`[ProgressPlugin] Total download subscribers: ${this.downloadSubscribers.size}`);
    return () => {
  ...
  emit(event: ProgressEvent) {
    ...
    console.log(`[ProgressPlugin] Notifying ${this.subscribers.size} general subscribers`);
    ...
    if (event.phase === "upload") {
      console.log(`[ProgressPlugin] Notifying ${this.uploadSubscribers.size} upload subscribers`);
      ...
      console.log(`[ProgressPlugin] Notifying ${this.downloadSubscribers.size} download subscribers`);
...
class ProgressTracker {
  ...
  updateUpload(loaded: number, total: number) {
    console.log(`[ProgressTracker] Upload progress update: ${loaded}/${total} bytes`);
  ...
  updateDownload(loaded: number, total: number) {
    console.log(`[ProgressTracker] Download progress update: ${loaded}/${total} bytes`);
...
function createUploadProgressStream(
  body: BodyInit,
  ...
  if (body instanceof ReadableStream) {
    ...
    total = body.size;
    console.log(`[ProgressPlugin] Body is Blob (size: ${total} bytes)`);
    stream = body.stream();
  } else if (body instanceof ArrayBuffer) {
    total = body.byteLength;
    console.log(`[ProgressPlugin] Body is ArrayBuffer (size: ${total} bytes)`);
    stream = new Blob([body]).stream();
  } else if (body instanceof FormData) {
    const blob = new Blob([body as any]);
    total = blob.size;
    console.log(`[ProgressPlugin] Body is FormData (size: ${total} bytes)`);
    stream = blob.stream();
  } else if (typeof body === "string") {
    const blob = new Blob([body]);
    total = blob.size;
    console.log(`[ProgressPlugin] Body is string (size: ${total} bytes)`);
    stream = blob.stream();
  } else {
    const blob = new Blob([body as any]);
    total = blob.size;
    console.log(`[ProgressPlugin] Body is unknown type (size: ${total} bytes)`);
    stream = blob.stream();
  }
    ...
    link.clientInterceptors.push(async (options) => {
      ...
      try {
        ...
        if (request.body) {
          ...
          modifiedRequest = {
            ...request,
            body: createUploadProgressStream(request.body, tracker),
          };
        }
