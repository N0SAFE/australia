...
export interface GetPathOptions {
   ...
   */
  serializeParams?: (input: any) => string
}

/**
 * ORPC Contract type with metadata
 */
type ContractWithMetadata<TInputSchema extends ZodType = ZodType> = {
  '~orpc': {
 ...
 */
function defaultSerializeParams(input: any): string {
  ...
  if (input && typeof input === 'object') {
    path = path.replace(/:([a-zA-Z_][a-zA-Z0-9_]*)/g, (match: string, paramName: string) => {
      if (paramName in input) {
        return encodeURIComponent(String(input[paramName]))
      }
 ...
 * ```
 */
export function hasRoute(contract: any): contract is ContractWithMetadata {
