...
export const SuggestionMenu = ({
  ...
  useEffect(() => {
    ...
    const suggestion = Suggestion({
      ...
      command({ editor, range, props }) {
        ...
        // Store the deletion info but don't execute it yet
        let deletionInfo: { startPosition: number; cursorPosition: number } | null = null
        ...
        if (!isMention) {
          ...
          deletionInfo = { startPosition, cursorPosition }
        }
        ...
        // Pass deletion info to onSelect so it can handle deletion + insertion atomically
        props.onSelect({ 
          editor, 
          range: rangeToUse, 
  ...
  const onSelect = useCallback(
    (item: SuggestionItem) => {
      ...
      if (internalCommand) {
        // Defer command execution to ensure the suggestion plugin
        // has fully cleaned up and unregistered event listeners
        // This prevents "Position X out of range" errors and
        // ensures upload nodes don't disappear after insertion
