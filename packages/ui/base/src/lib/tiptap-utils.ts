...
export function findNodePosition(props: {
  ...
  if (node) {
    const search: { result: { pos: number; node: TiptapNode } | null } = {
      result: null,
    }

    editor.state.doc.descendants((currentNode, pos) => {
      if (currentNode === node) {
        search.result = { pos, node: currentNode }
        return false
      }
      return true
    })
...
export function isNodeTypeSelected(
  ...
  if (selection instanceof NodeSelection) {
    ...
    return nodeTypeNames.includes(selectedNode.type.name)
  }
...
export const handleImageUpload = async (
  ...
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(
      `File size exceeds maximum allowed (${String(MAX_FILE_SIZE / (1024 * 1024))}MB)`
    )
  }
 ...
 */
export function updateNodesAttr<V = unknown>(
  tr: Transaction,
